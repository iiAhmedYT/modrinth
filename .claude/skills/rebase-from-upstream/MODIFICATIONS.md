...
| `apps/app/src/api/ads.rs` | A minimal plugin that registers the same command names (`init_ads_window`, `show_ads_window`, `hide_ads_window`, `record_ads_click`, `open_link`, `get_ads_personalization`) but does nothing. No `AD_LINK`, no webview construction, no refresh loop. |
