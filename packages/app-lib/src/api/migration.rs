...
pub struct MigrationCandidate {
    ...
    /// Absolute path to where Meverinth will copy the data.
...
pub async fn find_modrinth_install_candidate(
    ...
    let our_dir =
        match DirectoryInfo::initial_settings_dir_path(meverinth_identifier) {
            Some(dir) => dir,
            None => return Ok(None),
        };

    if has_state_marker(&our_dir) {
        return Ok(None);
    }
    ...
    // Refuse to offer a migration when the source and destination resolve to
    // the same directory. That can happen if the Meverinth build was published
    // with the upstream `ModrinthApp` identifier, or if the user set the
    // `THESEUS_CONFIG_DIR` env var to Modrinth App's data folder — in either
    // case, "copying" the folder into itself would corrupt the install.
    if paths_refer_to_same_dir(&modrinth_dir, &our_dir) {
        tracing::warn!(
            "Skipping migration offer: source and destination resolve to the same directory ({}). \
             Check the Meverinth app identifier and any THESEUS_CONFIG_DIR override.",
            modrinth_dir.display()
        );
        return Ok(None);
    }
    ...
    Ok(Some(MigrationCandidate {
        ...
        destination_path: our_dir,
        estimated_size_bytes,
    }))
}

/// Compares two directory paths, canonicalizing when possible so
/// case-insensitive filesystems (Windows / default macOS) and symlink
/// wrappers don't hide an identity match.
...
pub async fn migrate_from_path(
    ...
    if paths_refer_to_same_dir(&source_path, &dest_dir) {
        return Err(ErrorKind::OtherError(format!(
            "Refusing to migrate: source and destination point to the same directory ({})",
            source_path.display()
        ))
        .into());
    }
